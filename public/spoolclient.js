...
var Client = (initObject) => {
    ...
    self.socketInit = () => {
        ...
        self.socket.on(MessageCodes.SM_PACK_INIT, (data) => {
            ...
            if (!self.firstInit) {
                if (self.onFirstLoad) self.onFirstLoad(self);
                self.firstInit = true;
            }
        });
        ...
        self.socket.on(MessageCodes.SM_PACK_UPDATE, (data) => {
            ...
            if (self.clientId && !self.clientObject) {
                self.clientObject = self.handler.getObject(
                    self.clientObjectFingerprint.objectType,
                    self.clientObjectFingerprint.id
                );
...
var GameArea = (width = 500, height = 500) => {
    ...
    SpoolRenderer.camera = self.camera;
...
var TextureManager = (spriteSheetInitObject, objectSheetInitObject) => {
    ...
    self.load = () => {
        ...
        keys.forEach((key) => {
            ...
            image.onload = () => {
                ...
                var r = self.spriteSheetInitObject[key].r
                    ? self.spriteSheetInitObject[key].r
                    : 1;
                var c = self.spriteSheetInitObject[key].c
                    ? self.spriteSheetInitObject[key].c
                    : 1;
    ...
    self.getSprite = (key, x = 0, y = 0) => {
        if (!self.spriteSheets[key]) {
            ...
            return self.spriteSheets[key].sprites[
                y * self.spriteSheets[key].columns + x
            ];
        }
    };
    ...
    self.prepareObjectSheets = () => {
        ...
        keys.forEach((key) => {
            ...
            var xx = SpoolMath.numberDefined(info.xx)
                ? info.xx
                : spriteSheet.columns - 1;
            var yy = SpoolMath.numberDefined(info.yy)
                ? info.yy
                : spriteSheet.rows - 1;
            ...
            for (var yyy = y; yyy <= yy; yyy += variantHeight) {
                for (var xxx = x; xxx <= xx; xxx += variantWidth) {
                    var temp = self.getSubSpriteSheet(
                        info.src,
                        xxx,
                        yyy,
                        xxx + variantWidth - 1,
...
cameraContainsEntity = (camera, obj) => {
    let point = ({ x, y, width, height } = camera.transformBounds(
        obj.x,
        obj.y,
        obj.radius * 2,
        obj.radius * 2
    ...
    if (
        0 <= point.x + point.width / 2 &&
        point.x - point.width / 2 <= camera.width * camera.scale &&
        0 <= point.y + point.height / 2 &&
        point.y - point.height / 2 <= camera.height * camera.scale
    ) {
        return true;
    }
...
var Grid = () => {
    ...
    self.render = (ctx, camera) => {
        ...
        while (column < (camera.x + camera.width) / CAMERA.scale) {
            ...
            let top = ({ x, y } = camera.transformPoint(topX, topY));
            topX = top.x;
            topY = top.y;

            let bottom = ({ x, y } = camera.transformPoint(bottomX, bottomY));
            bottomX = bottom.x;
            bottomY = bottom.y;
        ...
        while (row < (camera.y + camera.width) / CAMERA.scale) {
            ...
            let left = ({ x, y } = camera.transformPoint(leftX, leftY));
            leftX = left.x;
            leftY = left.y;

            let right = ({ x, y } = camera.transformPoint(rightX, rightY));
            rightX = right.x;
            rightY = right.y;
        ...
        if (camera.followObject) {
            if (self.objectType in self.handler.objects) {
                ...
                for (id in objects) {
                    ...
                    var angle =
                        Math.PI * 2 -
                        SpoolMath.objGlobalAngle(camera.followObject, obj) +
                        camera.rotation;

                    ctx.fillStyle = obj.color;

                    let p = ({ x, y } = SpoolMath.polarPoint(
                        (camera.width / 2) * camera.scale,
                        (camera.height / 2) * camera.scale,
                        200,
                        angle
                    ));
                    ...
                    var ox = p.x;
                    var oy = p.y;
                    let a = ({ x, y } = SpoolMath.polarPoint(
                        p.x,
                        p.y,
                        20,
                        angle + (Math.PI * 3) / 4
                    ));
                    let b = ({ x, y } = SpoolMath.polarPoint(
                        ox,
                        oy,
                        20,
                        angle + (Math.PI * 5) / 4
...
var ClientHandler = (chunkSize, client, pureLocal = false) => {
     ...
     */
    self.render = (ctx, camera) => {
        var min_x = Math.floor(
            (camera.x - camera.width / 2 - 100) / self.chunkSize
        );
        var min_y = Math.floor(
            (camera.y - camera.height / 2 - 100) / self.chunkSize
        );
        var max_x = Math.floor(
            (camera.x + camera.width / 2 + 100) / self.chunkSize
        );
        var max_y = Math.floor(
            (camera.y + camera.height / 2 + 100) / self.chunkSize
        );
        ...
        for (chunkKey in chunks) {
            ...
            if (chunk.sprite) {
                var bounds = camera.transformBounds(
                    chunk.x * self.chunkSize,
                    (chunk.y + 1) * self.chunkSize,
                    self.chunkSize,
                    self.chunkSize
                );
                ctx.drawImage(
            ...
            for (key in objects) {
                for (id in objects[key]) {
                    ...
                    if (obj.bakeIn) {
                        if (
                            obj.bakedIn
                                ? !obj.bakedIn.includes(chunk.key)
                                : true
                        ) {
        ...
        for (key in bakingObjects) {
            if (key in self.chunks) {
                ...
                var sortedChunkLayers = Object.keys(bakingObjects[key]).sort(
                    (a, b) => {
    ...
    self.removeBulk = (data) => {
        for (key in data) {
            if (key in self.objects) {
                // If object type in handler (PLAYER type etc.)
                for (var i = 0; i < data[key].length; i++) {
                    if (data[key][i] in self.objects[key]) {
                        // If objects id is in the sub-objects list (If object is in handler)
...
var Camera = (initPack = {}) => {
    ...
    self.update = () => {
        if (self.followObject) {
            ...
            if (self.lerpRotation) {
                self.rotation = SpoolMath.lerpRotation(
                    self.rotation,
                    self.followObject.rotation - Math.PI / 2,
                    self.rotationSpeed
                );
            }

            if (self.lerpSpeedToScale) {
                var vel = self.followObject.velocity
                    ? self.followObject.velocity
                    : 0;
            }
            ...
            if (self.lerp) {
                self.x = SpoolMath.lerp(
                    self.x,
                    self.followObject.x + self.offsetX,
                    self.followSpeed
                );
                self.y = SpoolMath.lerp(
                    self.y,
                    self.followObject.y + self.offsetY,
                    self.followSpeed
                );
            }
    ...
    self.transformBounds = (x, y, width, height) => {
        let point = ({ x, y } = self.transformPoint(x, y));
    ...
    self.transformPoint = (x, y) => {
        ...
        var newX =
            self.scaleX *
            ((x - self.x) * cos - (-y + self.y) * sin + self.width / 2);
        var newY =
            self.scaleY *
            ((x - self.x) * sin + (-y + self.y) * cos + self.height / 2);
    ...
    self.inverseTransformPoint = (x, y) => {
        ...
        var b =
            self.y -
            (-sin * (x / self.scaleX - self.width / 2) +
                cos * (y / self.scaleY - self.height / 2));
        var a =
            cos * (x / self.scaleX - self.width / 2) +
            sin * (y / self.scaleY - self.height / 2) +
            self.x;
...
var ClientEntity = (initPack, extending = null) => {
    ...
    self.renderNtagon = (
        ...
        ctx.moveTo(
            point.x - r * Math.cos(startAngle),
            point.y - r * Math.sin(startAngle)
        );
    ...
    self.renderSprite = (
        ...
        if (sprite) {
            ...
            if (!hardBounds) {
                ...
                var offsetBounds = camera.transformBounds(
                    x,
                    y,
                    offsetX,
                    offsetY
    ...
    self.getMovementAnimationSpriteIndex = (
        ...
        if (moving) {
            ...
            var row =
                (Math.floor(angleCoef * animationRowsNumber) %
                    animationRowsNumber) +
                1;
        } else {
            var row = 0;
...
var Point = (initObject) => {
    ...
    self.render = (ctx, camera) => {
        ...
        let { x, y, width } = camera.transformBounds(self.x, self.y, 3, 3);
...
var MouseListener = (client, initObject) => {
    ...
    self.gamePlaneMouseButtonEvent = (event, x, y) => {
        if (
            self.activeButtons.includes(event.button) &&
            self.activeTypes.includes(event.type)
        ) {
            var value =

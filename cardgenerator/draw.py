...
def export_surface_area(surface, coors, size, file_name):
    new_surface = surface.subsurface(
        pygame.Rect(coors[0], coors[1], size[0], size[1]))
...
def render_stack_of_cards(json, items_tiles, multiplier):
    surface = pygame.Surface(
        [1500*multiplier, 1000*multiplier], pygame.SRCALPHA, 32)
    surface = surface.convert_alpha()

    weapon_card_border_img = pygame.image.load("card_border.png")
    spell_card_border_img = pygame.image.load(
        "card_borders/greyscale/border_10.png")
    trinket_card_border_img = pygame.image.load("dark_green_border.png")
    special_card_border_img = pygame.image.load(
        "card_borders/classic/border_265.png")
    top_background_img = pygame.image.load("bg_vignetta.png")
    weapon_bottom_background_img = pygame.image.load("card_bottom_bg.png")
    spell_bottom_background_img = pygame.image.load(
        "card_bottom_bgs/greyscale/bg_13.png")
    ...
    special_bottom_background_img = pygame.image.load(
        "card_bottom_bgs/classic/bg_0.png")
    ...
    middle_left_crystal_coors = (
        left_crystal_coors[0] + crystal_size[0]/2, left_crystal_coors[1] + crystal_size[1]/2)
    top_crystal_coors = [i*multiplier for i in (39, 0)]
    middle_top_crystal_coors = (
        top_crystal_coors[0] + crystal_size[0]/2, top_crystal_coors[1] + crystal_size[1]/2)
    right_crystal_coors = [i*multiplier for i in (78, 43)]
    middle_right_crystal_coors = (
        right_crystal_coors[0] + crystal_size[0]/2, right_crystal_coors[1] + crystal_size[1]/2)
    ...
    for card in json["cards"]:
        ...
        x, y = ((card["cardTileIndex"] % items_tiles_width) * total_card_size[0],
                (card["cardTileIndex"] // items_tiles_width) * total_card_size[1])

        draw_image(surface, top_background_img,
                   (top_hexagon_middle_coors[0] + x, top_hexagon_middle_coors[1] + y), background_size, centered=True)

        if card["type"] == "weapon":
            draw_image(surface, weapon_bottom_background_img,
                       (card_base_coors[0] + x, card_base_coors[1] + y), card_base_size)
            draw_image(surface, weapon_card_border_img,
                       (card_base_coors[0] + x, card_base_coors[1] + y), card_base_size)
        elif card["type"] == "spell":
            draw_image(surface, spell_bottom_background_img,
                       (card_base_coors[0] + x, card_base_coors[1] + y), card_base_size)
            draw_image(surface, spell_card_border_img,
                       (card_base_coors[0] + x, card_base_coors[1] + y), card_base_size)
        elif card["type"] == "trinket":
            draw_image(surface, trinket_bottom_background_img,
                       (card_base_coors[0] + x, card_base_coors[1] + y), card_base_size)
            draw_image(surface, trinket_card_border_img,
                       (card_base_coors[0] + x, card_base_coors[1] + y), card_base_size)
        elif card["type"] == "special":
            draw_image(surface, special_bottom_background_img,
                       (card_base_coors[0] + x, card_base_coors[1] + y), card_base_size)
            draw_image(surface, special_card_border_img,
                       (card_base_coors[0] + x, card_base_coors[1] + y), card_base_size)

        draw_image(surface, title_img,
                   (card_base_coors[0] + x, card_base_coors[1] + y), card_base_size)

        img_x = (card["textureTileIndex"] %
                 items_tiles_width) * item_tile_size[0]
        img_y = (card["textureTileIndex"] //
                 items_tiles_width) * item_tile_size[1]
        img = get_subsurface(items_tiles, (img_x, img_y), item_tile_size)
        draw_image(surface, img, (top_hexagon_middle_coors[0] + x,
                                  top_hexagon_middle_coors[1] + y), rendered_item_size, centered=True)

        if card["type"] == "weapon":
            draw_image(surface, left_crystal_img,
                       (left_crystal_coors[0]+1 + x, left_crystal_coors[1] + y), crystal_size)
            font_size = crystal_font.size(str(card["dmg"]))
            font_x = middle_left_crystal_coors[0] + x - \
                font_size[0]/2 + crystal_number_offset[0]
            font_y = middle_left_crystal_coors[1] + y - \
                font_size[1]/2 + crystal_number_offset[1]
            renderText(surface, str(
                card["dmg"]), (font_x+1, font_y), crystal_font, centered=False, color=font_color)
            renderText(surface, str(
                card["dmg"]), (font_x-1, font_y), crystal_font, centered=False, color=font_color)
            renderText(surface, str(
                card["dmg"]), (font_x, font_y+1), crystal_font, centered=False, color=font_color)
            renderText(surface, str(
                card["dmg"]), (font_x, font_y-1), crystal_font, centered=False, color=font_color)
            renderText(surface, str(card["dmg"]), (font_x, font_y),
                       crystal_font, centered=False, color=(255, 255, 255))

        draw_image(surface, top_crystal_img,
                   (top_crystal_coors[0] + x, top_crystal_coors[1] + y), crystal_size)
        font_size = crystal_font.size(str(card["cost"]))
        font_x = middle_top_crystal_coors[0] + x - \
            font_size[0]/2 + crystal_number_offset[0]
        font_y = middle_top_crystal_coors[1] + y - \
            font_size[1]/2 + crystal_number_offset[1]
        renderText(surface, str(card["cost"]), (font_x+1, font_y),
                   crystal_font, centered=False, color=font_color)
        renderText(surface, str(card["cost"]), (font_x-1, font_y),
                   crystal_font, centered=False, color=font_color)
        renderText(surface, str(card["cost"]), (font_x, font_y+1),
                   crystal_font, centered=False, color=font_color)
        renderText(surface, str(card["cost"]), (font_x, font_y-1),
                   crystal_font, centered=False, color=font_color)
        renderText(surface, str(card["cost"]), (font_x, font_y),
                   crystal_font, centered=False, color=(255, 255, 255))
        ...
        draw_image(surface, right_crystal_img,
                   (right_crystal_coors[0] + x, right_crystal_coors[1] + y), crystal_size)
        font_size = crystal_font.size(str(card["range"]))
        font_x = middle_right_crystal_coors[0] + x - \
            font_size[0]/2 + crystal_number_offset[0]
        font_y = middle_right_crystal_coors[1] + y - \
            font_size[1]/2 + crystal_number_offset[1]
        renderText(surface, str(card["range"]), (font_x+1, font_y),
                   crystal_font, centered=False, color=font_color)
        renderText(surface, str(card["range"]), (font_x-1, font_y),
                   crystal_font, centered=False, color=font_color)
        renderText(surface, str(card["range"]), (font_x, font_y+1),
                   crystal_font, centered=False, color=font_color)
        renderText(surface, str(card["range"]), (font_x, font_y-1),
                   crystal_font, centered=False, color=font_color)
        renderText(surface, str(card["range"]), (font_x, font_y),
                   crystal_font, centered=False, color=(255, 255, 255))

        title_font = font_of_max_size(
            title_size, card["name"], title_font_name)
        renderText(surface, str(card["name"]), (title_middle_coors[0] + x,
                                                title_middle_coors[1] + y), title_font, centered=True, color=font_color)
        render_text_into_box(surface, card["description"], description_optimal_font_ascent, (bottom_hexagon_middle_coors[0] + x,
                                                                                             bottom_hexagon_middle_coors[1] + y), bottom_size, description_font_name, font_color, 0, centered=True, center_align=True)

        #pygame.draw.rect(surface, (255, 0, 0), [bottom_hexagon_middle_coors[0], bottom_hexagon_middle_coors[1], 1, 1])
        export_surface_area(surface, coors, total_card_size,
                            "cards/" + card["cardID"] + ".png")

    export_surface_area(surface, (0, 0), (204*5, 380*5),
...
items_tiles = pygame.image.load("./../items.png")
